import type { PipelineStage, StepResult, PipelineResult } from '~/types/pipeline';

export function processText(input: string, stages: PipelineStage[]): PipelineResult {
  ...
  try {
    ...
    if (stage.useRegex) {
      ...
      if (stage.wordBoundary) {
        ...
        const searchValue = stage.caseSensitive ? pattern : new RegExp(escapeRegExp(pattern), 'gi');
